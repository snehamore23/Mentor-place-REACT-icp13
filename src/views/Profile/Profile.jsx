        ...
        <aside className="pp-aside">
          <div className="pp-card pp-hero-card">
            ...
            <div className="pp-avatar-wrap">
              {photo
                ? <img src={photo} alt="profile" className="pp-avatar pp-avatar-img" />
                : <div className="pp-avatar">{initials}</div>
              }
              <button className="pp-photo-btn" onClick={() => fileRef.current.click()} title="Upload photo">
                📷
              </button>
              <input
                ref={fileRef}
          ...
          <div className="pp-section">
            ...
            <div className="pp-tasks">
              {features.map((f) => {
                ...
                return (
                  <div key={f.key} className={`pp-task ${isDone ? "pp-task-done" : ""}`} onClick={() => navigate(f.path)}>
                    <div className="pp-task-icon" style={{ background: f.color + "18", color: f.color }}>{f.icon}</div>
                    <div className="pp-task-info">
                      <span className="pp-task-title">{f.title}</span>
                      <div className="pp-task-bar-track">
                        <div className="pp-task-bar-fill" style={{ width: isDone ? "100%" : "0%", background: f.color }} />
                      </div>
                    </div>
